...
describe('translate', function () {
  ...
  describe('when called', function () {
    describe('with a non-empty string or an array as first argument', function () {
      ...
      describe('with the default locale present', function () {
        ...
        describe('with a current scope present', function () {
          it('generates the correct normalized keys', function () {
            instance.withScope('other', function () {
              assert.equal(
                instance.translate('foo'),
                'missing translation: en.other.foo',
              );
            });
          });
        });
        ...
        describe('with a scope provided as option', function () {
          it('generates the correct normalized keys', function () {
            assert.equal(
              instance.translate('foo', { scope: 'other' }),
              'missing translation: en.other.foo',
            );
          });
        });
          ...
          it('generates the correct normalized keys', function () {
            instance.withLocale('de', function () {
              instance.withScope('other', function () {
                assert.equal(
                  instance.translate('foo'),
                  'missing translation: de.other.foo',
                );
              });
            });
          });
      ...
      describe('with a locale provided as option', function () {
        describe('without a current scope or provided scope option', function () {
          it('generates the correct normalized keys', function () {
            assert.equal(
              instance.translate('foo', { locale: 'de' }),
              'missing translation: de.foo',
            );
          });
        });
        ...
        describe('with a scope provided as option', function () {
          it('generates the correct normalized keys', function () {
            assert.equal(
              instance.translate('foo', { locale: 'de', scope: 'other' }),
              'missing translation: de.other.foo',
            );
          });
        });
      ...
      describe('with a translation for the key present', function () {
        it('returns that translation', function () {
          ...
          assert.equal(
            instance.translate('bar.baz.bam', { scope: 'foo' }),
            'boo',
          );
          assert.equal(
            instance.translate('baz.bam', { scope: 'foo.bar' }),
            'boo',
          );
          assert.equal(
            instance.translate('bam', { scope: 'foo.bar.baz' }),
            'boo',
          );
          ...
          // strange looking
          assert.equal(
            instance.translate(['..foo.bar', 'baz', '', 'bam']),
            'boo',
          );
          assert.equal(
            instance.translate(['bar', 'baz..bam.'], { scope: '.foo' }),
          ...
          assert.equal(
            instance.translate(['baz', null, 'bam'], { scope: 'foo.bar.' }),
            'boo',
          );
          assert.equal(
            instance.translate('bam...', {
              scope: [null, 'foo..bar', '', 'baz'],
            }),
            'boo',
          );
        });
        ...
        describe('with a `separator` provided as option', function () {
          it('correctly returns single array with key', function () {
            instance.registerTranslations('en', {
              'long.key.with.dots.in.name':
                "Key with dots doesn't get split and returns correctly",
              another: {
                key: 'bar',
              },
              mixed: {
            ...
            assert.equal(
              instance.translate('long.key.with.dots.in.name', {
                separator: '-',
              }),
              "Key with dots doesn't get split and returns correctly",
            );
            assert.equal(
              instance.translate('long.key.with.dots.in.name.not-found', {
                separator: '-',
              }),
              'missing translation: en-long.key.with.dots.in.name.not-found',
            );
            assert.equal(
              instance.translate('another-key', { separator: '-' }),
              'bar',
            );
            assert.equal(
              instance.translate('mixed-dots.and-separator', {
                separator: '-',
              }),
              'bingo',
            );
          ...
          it('correctly returns nested key when using `*` as seperator', function () {
            instance.registerTranslations('en', {
              long: { key: { with: { dots: { in: { name: 'boo' } } } } },
            });
          ...
          describe('by default', function () {
            it('interpolates these options into the translated value', function () {
              ...
              assert.equal(
                ...
                'Hi Paul! See you later!',
              );

              instance.registerTranslations('en', {
                foo: 'Hello %(users[0].name)s and %(users[1].name)s!',
              });
              assert.equal(
                instance.translate('foo', {
                  users: [{ name: 'Molly' }, { name: 'Polly' }],
                }),
                'Hello Molly and Polly!',
              );
            });
            ...
            it('interpolates the registered interpolations into the translated value', function () {
              ...
              instance.registerInterpolations({
                app_name: 'My Cool App',
                question: 'How are you today?',
              });
              instance.registerTranslations('en', {
                greeting: 'Welcome to %(app_name)s, %(name)s! %(question)s',
              });

              assert.equal(
                instance.translate('greeting', { name: 'Martin' }),
                'Welcome to My Cool App, Martin! How are you today?',
              );
              assert.equal(
                instance.translate('greeting', {
                  name: 'Martin',
                  app_name: 'The Foo App',
                }),
                'Welcome to The Foo App, Martin! How are you today?',
              );
        ...
        describe('with the keepTrailingDot setting set to true', function () {
          it('returns the translation for keys that contain a trailing dot', function () {
            instance.registerTranslations('fr', {
              foo: { bar: 'baz', 'With a dot.': 'Avec un point.' },
              'dot.': 'point.',
            });
            ...
            instance.withLocale('fr', function () {
              ...
              assert.equal(
                instance.translate('foo.With a dot.'),
                'Avec un point.',
              );
              ...
              assert.equal(
                ...
                'missing translation: fr.foo.bar.',
              );
              assert.equal(
                instance.translate('foo.With a dot..'),
                'missing translation: fr.foo.With a dot..',
              );
              assert.equal(
                instance.translate('foo.With. a dot.'),
                'missing translation: fr.foo.With. a dot.',
              ...
              assert.equal(
                instance.translate('dot..'),
                'missing translation: fr.dot..',
              );
            });
          });
        });
      ...
      describe('with a translation for a prefix of the key present', function () {
        it('returns the remaining translation part', function () {
          ...
          assert.deepEqual(
            instance.translate('baz', { scope: ['foo', 'bar'] }),
            { zero: 'no items', one: 'one item', other: '%(count)s items' },
          );
        });
      });
      ...
      describe('with an array-type translation for the key present', function () {
        it('returns the array that key points to', function () {
          instance.registerTranslations('en', {
            foo: { bar: { baz: [1, 'A', 0.42] } },
          });
          assert.deepEqual(
            instance.translate(['bar', 'baz'], { scope: 'foo' }),
            [1, 'A', 0.42],
          );
        });
      });
      ...
      describe('with a function-type fallback present', function () {
        it('returns the array that key points to', function () {
          ...
          assert.equal(
            instance.translate('i_dont_exist', { fallback: myFunc }),
            'Here I am!',
          );
          assert.equal(
            instance.translate('i_dont_exist', {
              fallback: myFunc2,
              object: 'you',
            }),
            'Here you are!',
          ...
          assert.equal(
            instance.translate('i_dont_exist', { fallback: myFunc2 }),
            'Here i_dont_exist are!',
          );
          assert.equal(
            instance.translate('i_dont_exist', { fallback: fallbacks }),
            'Here I am!',
          );
        });
      });
      ...
      describe('without a translation for the key present', function () {
        it('returns a string "missing translation: %(locale).%(scope).%(key)"', function () {
          assert.deepEqual(
            instance.translate('bar', { locale: 'unknown', scope: 'foo' }),
            'missing translation: unknown.foo.bar',
          );
        });
        ...
        describe('with a `fallback` provided as option', function () {
          ...
          it('translates the fallback if given as "symbol" or array', function () {
            ...
            assert.equal(
              instance.translate('missing', { fallback: 'default' }),
              'default',
            );
            assert.equal(
              instance.translate('missing', { fallback: ':foo.bar' }),
              'bar',
            );
            assert.equal(
              instance.translate('missing', { fallback: ':bar', scope: 'foo' }),
              'bar',
            );
            assert.equal(
              instance.translate('missing', {
                fallback: [':also_missing', ':foo.bar'],
              }),
              'bar',
            );
            assert.matches(
              instance.translate('missing', {
                fallback: [':also_missing', ':foo.missed'],
              }),
              /missing translation/,
            );
          });
        ...
        describe('with a global `fallbackLocale` present', function () {
          it('returns the entry of the fallback locale', function () {
            ...
            assert.equal(
              instance.translate('baz', { locale: 'foo', scope: 'bar' }),
              'missing translation: foo.bar.baz',
            );
            assert.equal(
              instance.translate('hello', { locale: 'foo', name: 'Martin' }),
              'missing translation: foo.hello',
            );
        ...
        describe('with multiple global `fallbackLocales` present', function () {
          it('returns the entry of the last fallback locale', function () {
            ...
            var previousFallbackLocale = instance.setFallbackLocale([
              'bar',
              'de',
            ]);
    ...
    describe('with global interpolate setting set to false', function () {
      it('will not interpolate', function () {
        ...
        assert.equal(
          instance.translate('hello', { interpolate: true }),
          'Hello from %(brand)s!',
        );
  ...
  describe('#getFallbackLocale', function () {
    ...
    it('returns the fallback locale stored in the registry', function () {
      assert.equal(
        ...
        instance._registry.fallbackLocales,
      );
    });
  ...
  describe('#getAvailableLocales', function () {
    ...
    it('returns the locales of the registered translations by default', function () {
      assert.deepEqual(
        ...
        Object.keys(instance._registry.translations),
      );
    });
  });
        ...
        console.error = function (message) {
          if (/EventEmitter memory leak/.test(message)) {
            assert.fail(
              null,
              null,
              'Node issues a warning about a possible memory leak',
              null,
            );
          } else {
  ...
  describe('#getKeyTransformer', function () {
    ...
    it('returns the setting stored in the registry', function () {
      assert.equal(
        instance.getKeyTransformer(),
        instance._registry.keyTransformer,
      );
    });
  });
  ...
  describe('#setKeyTransformer', function () {
    ...
    it('uses the custom key transformer when translating', function () {
      ...
      var translation = instance.translate('FOO', {
        locale: 'xx',
        bingo: 'bongo',
      });
  ...
  describe('#getMissingEntryGenerator', function () {
    ...
    it('returns the setting stored in the registry', function () {
      assert.equal(
        instance.getMissingEntryGenerator(),
        instance._registry.generateMissingEntry,
      );
    });
  });
      ...
      describe('providing a `format` key in the options', function () {
        ...
        describe('with format = "long"', function () {
          it('returns the long localization for that date', function () {
            ...
            assert.equal(
              result,
              'Wednesday, February 5th, 2014 22:09:04 -06:00',
            );
          });
        });
        ...
        describe('providing a `format` key in the options', function () {
          ...
          describe('with format = "long"', function () {
            it('returns the long localization for that date', function () {
              ...
              assert.equal(
                result,
                'Mittwoch, 5. Februar 2014, 22:09:04 -06:00',
              );
            });
          });
        ...
        describe('providing a `format` key in the options', function () {
          ...
          describe('with format = "long"', function () {
            it('returns the long localization for that date', function () {
              ...
              assert.equal(
                result,
                'Quarta-feira, 5 de Fevereiro de 2014 às 22:09:04 -06:00',
              );
            });
          });
  ...
  describe('#registerTranslations', function () {
    ...
    it('merges the passed arguments correctly into the registry', function () {
      ...
      var expected = {
        foo: { bar: { baz: 'bingo', bam: 'boo' }, bing: { bong: 'beng' } },
      };
      assert.deepEqual(instance._registry.translations, expected);
  ...
  describe('#registerInterpolations', function () {
    ...
    it('merges the passed arguments correctly into the registry', function () {
      ...
      assert.deepEqual(instance._registry.interpolations, {
        foo: 'yes',
        bar: 'no',
      });
  ...
  describe('explicitly checking the examples of the README', function () {
    it('passes all tests', function () {
      ...
      assert.deepEqual(translate('damals'), {
        about_x_hours_ago: {
          one: 'about one hour ago',
          other: 'about %(count)s hours ago',
        },
      ...
      translate.registerTranslations(
        'de',
        JSON.parse(
          '{"my_project": {"greeting": "Hallo, %(name)s!","x_items": {"one": "1 Stück", "other": "%(count)s Stücke"}}}',
        ),
      );
      ...
      assert.equal(
        translate.localize(date, { format: 'short' }),
        '21 Feb 13:46',
      );
      assert.equal(
        translate.localize(date, { format: 'long' }),
        'Friday, February 21st, 2014 13:46:24 +01:00',
      );

      assert.equal(
        translate.localize(date, { type: 'date' }),
        'Fri, 21 Feb 2014',
      );
      assert.equal(
        translate.localize(date, { type: 'date', format: 'short' }),
        'Feb 21',
      );
      assert.equal(
        translate.localize(date, { type: 'date', format: 'long' }),
        'Friday, February 21st, 2014',
      );

      assert.equal(translate.localize(date, { type: 'time' }), '13:46');
      assert.equal(
        translate.localize(date, { type: 'time', format: 'short' }),
        '13:46',
      );
      assert.equal(
        translate.localize(date, { type: 'time', format: 'long' }),
        '13:46:24 +01:00',
      );

      assert.equal(
        translate.localize(date, { locale: 'de' }),
        'Fr, 21. Feb 2014, 13:46 Uhr',
      );
      ...
      assert.equal(
        translate('my_namespace.greeting', {
          visitor: 'Martin',
          app_name: 'The Foo App',
        }),
